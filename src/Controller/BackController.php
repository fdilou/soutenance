...
#[IsGranted('ROLE_ADMIN')]// pour se mettre en role administrateur
class BackController extends AbstractController
{
    ...
    public function ajoutProduit(Request $request, EntityManagerInterface $manager): Response
    {
        ...
        // gràce à la méthode handleRequest de notre objet de formulaire, il charge à présent l'objet Product de données receptionnées du formulaire présentent dans notre objet request (Request étant la classe de symfony qui récupère la majeur partie des données de superGlobale =>$_GET, $_POST ...)
    ...
    #[Route('/ajoutSousCategories', name: 'ajoutSousCategories')]
    public function ajoutSousCategories(Request $request, EntityManagerInterface $manager): Response
    {
        $sous_categories = new SubCategory();

        $form = $this->createForm(SubCategoryType::class, $sous_categories);

        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {

            $manager->persist($sous_categories);
            $manager->flush();
            $this->addFlash('success', 'Sous-catégorie créée');
            return $this->redirectToRoute('gestionSousCategories');

        }


        return $this->render('back/ajoutSousCategories.html.twig', [
            'form' => $form->createView()
        ]);
    }

    #[Route('/editSousCategories/{id}', name: 'editSousCategories')]
    public function editSousCategories(SubCategory $subCategory, Request $request, EntityManagerInterface $manager): Response
    {
        ...
        return $this->render('back/editSousCategories.html.twig', [
            'form' => $form->createView()
        ]);
    }

    #[Route('/gestionSousCategories', name: 'gestionSousCategories')]
    public function gestionSousCategories(SubCategoryRepository $repository): Response
    {
        $subcategories = $repository->findAll();


        return $this->render('back/gestionSousCategories.html.twig', [
            'subCategories' => $subcategories
        ]);
    }


    #[Route('/deleteSousCategories/{id}', name: 'deleteSousCategories')]
    public function deleteSousCategories(EntityManagerInterface $manager, SubCategory $subCategory): Response
    {
    ...
    public function ajoutEntreprise(Request $request, EntityManagerInterface $manager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()){
            ...
            $manager->flush();
            $this->addFlash('success', 'Partenariat créé');
    ...
    public function editEntreprise(Request $request, Entreprise $entreprise, EntityManagerInterface $manager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()){
            ...
            $manager->persist($entreprise);
            $manager->flush();
            $this->addFlash('success', 'Partenariat modifié');
            return $this->redirectToRoute('gestionEntreprise');
    ...
    public function deleteEntreprise(Entreprise $entreprise, EntityManagerInterface $manager): Response
    {
        ...
        $this->addFlash('success', 'Partenariat supprimé');
    ...
    public function editAvis(Avis $avis, Request $request, EntityManagerInterface $manager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', 'Avis modifiée');
            return $this->redirectToRoute('gestionAvis');
